...
class Data():
    ...
    def find_data_per_node_per_class(self):
    	...
    	Finds the data based on node
    	"""
    	return {node:{cls:[datapoint for datapoint in self.data_per_node[node] if int(datapoint[1]) == cls] for cls in range(self.parameters['Number_of_classes'])} for node in range(1, self.parameters['Number_of_nodes']+1)}
    ...
    def find_summary_statistics(self):
    	...
    	self.summary_statistics['Mean_Waiting_Times_per_Node'] = {node:self.mean_waits(self.data_per_node[node]) for node in range(1,self.parameters['Number_of_nodes']+1)}
    	self.summary_statistics['Mean_Waiting_Times_per_Class'] = {cls:self.mean_waits(self.data_per_class[cls]) for cls in range(self.parameters['Number_of_classes'])}
    	self.summary_statistics['Mean_Waiting_Times_per_Node_per_Class'] = {node: {cls:self.mean_waits(self.data_per_node_per_class[node][cls]) for cls in range(self.parameters['Number_of_classes'])} for node in range(1,self.parameters['Number_of_nodes']+1)}
    	self.summary_statistics['Mean_Visits_per_Node'] = {node:self.mean_visits(self.data_per_node[node]) for node in range(1,self.parameters['Number_of_nodes']+1)}
    	self.summary_statistics['Mean_Customers_per_Node'] = {node:self.mean_customers(self.data_per_node[node]) for node in range(1,self.parameters['Number_of_nodes']+1)}
    ...
    def time_series_num_custs(self, data):
        """
        ...
        time_series = []
        time_seriescls = [[] for cls in range(self.parameters['Number_of_classes'])]
        for data_point in sorted_arrivals_and_exits:
            if data_point[1] == 'a':
                current_number_of_customers += 1
                time_series.append((data_point[0], current_number_of_customers))
                for cls in range(self.parameters['Number_of_classes']):
                    if data_point[2] == cls:
                        cls_customers[cls] += 1
                    time_seriescls[cls].append((data_point[0], cls_customers[cls]))
            elif data_point[1] == 'd':
                current_number_of_customers -= 1
                time_series.append((data_point[0], current_number_of_customers))
                for cls in range(self.parameters['Number_of_classes']):
        ...
        time_series_dictionary = {cls:[point[1] for point in time_seriescls[cls]] for cls in range(self.parameters['Number_of_classes'])}
        time_series_dictionary['Overall'] = [point[1] for point in time_series]
        time_series_dictionary['Time'] = [point[0] for point in time_series]
        return time_series_dictionary

    def plot_time_series(self, ts_dict, title="My Title"):
        """
        Plots the time series of customers
        """
        fig, ax = plt.subplots()
        box = ax.get_position()
        ax.set_position([box.x0, box.y0, box.width * 0.8, box.height])
        plt.plot(ts_dict['Time'], ts_dict['Overall'], 'black', label='All classes')
        for cls in range(self.parameters['Number_of_classes']):
            plt.plot(ts_dict['Time'], ts_dict[cls], label='Class '+str(cls))
        plt.xlabel('Time')
