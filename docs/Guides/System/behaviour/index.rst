...
Custom behaviour can be obtained by writing new :code:`Node`, :code:`ArrivalNode`, :code:`ExitNode`, :code:`Individual`, and/or :code:`Server` classes, that inherit from the original :code:`ciw.Node`, :code:`ciw.ArrivalNode`, :code:`ciw.Individual` and :code:`ciw.Server` classes respectively, that introduce new beahviour into the system.
...
- :code:`ExitNode`: the node class used to collect customers as they leave the system.
...
These new classes can be used with the Simulation class by using the keyword arugments :code:`node_class`, :code:`arrival_node_class`, :code:`exit_node_class`, :code:`individual_class`, and :code:`server_class`.
