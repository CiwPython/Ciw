...
class TestArrivalNode(unittest.TestCase):
    ...
    def test_have_event_method(self):
        ...
        Q = ciw.Simulation(ciw.create_network(
            'ciw/tests/testing_parameters/params.yml'))
        N = ciw.ArrivalNode(Q)
        self.assertEqual([str(obj) for obj in Q.transitive_nodes[0].all_individuals], [])
        self.assertEqual([str(obj) for obj in Q.transitive_nodes[1].all_individuals], [])
        self.assertEqual([str(obj) for obj in Q.transitive_nodes[2].all_individuals], [])
        self.assertEqual([str(obj) for obj in Q.transitive_nodes[3].all_individuals], [])
        self.assertEqual(round(N.next_event_date, 5), 0.00105)
        self.assertEqual(N.next_node, 1)
