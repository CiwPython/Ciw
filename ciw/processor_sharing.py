...
class PSNode(Node):
    ...
    def __init__(self, id_, simulation):
        ...
        self.ps_threshold = self.simulation.network.service_centres[
            id_ - 1
        ].ps_threshold
    ...
    def update_all_service_end_dates(self):
        ...
        for ind in inds_in_service:
            ...
            if self.last_occupancy > 0:
                share_completed = (self.ps_threshold * current_period) / max(
                    self.last_occupancy, self.ps_threshold
                )
            else:
                share_completed = 0
            ind.time_left -= share_completed
            ind.service_end_date = self.simulation.current_time + (
                (ind.time_left * max(next_occupancy, self.ps_threshold))
                / self.ps_threshold
            )
